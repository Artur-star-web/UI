...
//    }
    ...
    fun testEmptyText() {
        ...
        device.findObject(By.text("CHANGE TEXT")).click()
        val result1 = device.findObject(By.res(MODEL_PACKAGE, "textToBeChanged")).text
        assertEquals("Текст должен остаться прежним при пустой строке", originalText, result1)

        // Тест 2: Только пробелы
        device.findObject(By.res(MODEL_PACKAGE, "userInput")).text = "   "
        device.findObject(By.text("CHANGE TEXT")).click()
        val result2 = device.findObject(By.res(MODEL_PACKAGE, "textToBeChanged")).text
        assertEquals("Текст должен остаться прежним при пробелах", originalText, result2)
    }
